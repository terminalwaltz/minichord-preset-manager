...
function PresetManager() {
    ...
    setPresetState((prev) => {
      ...
      console.log(
        `>> Bulk edit: Setting ${bulkEdits
          ...
          .join(", ")} for presets ${selected.length > 0 ? selected.map(i => i + 1).join(",") : "all"}`
      );
      ...
      if (selected.length === 1) {
        ...
        (async () => {
          try {
            ...
            if (success) {
              console.log(`>> Bulk edit upload successful for preset ${presetIndex + 1}`);
              showUploadSuccess(`Bulk edit applied and uploaded successfully to preset ${presetIndex + 1}`);
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmed = window.confirm(
      "This action will overwrite your current presets, make sure to export a JSON backup if necessary. Are you sure?"
    );
    if (!confirmed) {
  ...
  const handleResetMemory = () => {
    if (!controller || !controller.isConnected()) {
      console.error(">> No device connected for reset memory");
      alert("No device connected");
      return;
    }
    const confirmed = window.confirm(
      "This action will reset your banks to factory presets, are you sure?"
    );
    if (!confirmed) {
      console.log(">> Reset memory cancelled by user");
      return;
    }
